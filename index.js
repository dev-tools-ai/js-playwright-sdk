...
async function getElementBox(element, page) {
  var element_location = await element.boundingBox();
...
async function matchBoundingBoxToWDIOElement(boundingBox, page) {
  ...
  for (const tagFilter of tagFilters) {
    var res = await matchBoundingBoxToWDIOElementCore(boundingBox, tagFilter, page);
    var score = res[0];
    var element = res[1];
  ...
  if (best_score >= 0.5) {
    ...
    var res = await matchBoundingBoxToWDIOElementCore(boundingBox, "*", page);
    var score = res[0];
    var element = res[1];
...
async function matchBoundingBoxToWDIOElementCore(boundingBox, tagFilter, page) {
  var elements = await page.$$("//" + tagFilter);
  var new_box = {
  ...
  for (const element of elements) {
    var element_box = getElementBox(element, page);
    var tag_name = element.getProperty('tagName');
...
async function register_devtools(page, tc_name, api_key = null, priority_tags = []) {
  ...
  const im = await sharp("screenshot.png");
  ...
  async function findByAI(selector) {
    const element_name = selector;
    var screenshot = (await page.screenshot()).toString("base64");
    ...
    if (interactive_mode) {
      ...
      if (tc_res.success && tc_res.predicted_element != null) {
        ...
        var real_elem = await matchBoundingBoxToWDIOElement(element_box, page);
        return real_elem;
      } else {
  ...
  page.findByAI = findByAI;
  page.orig_locator = page.locator;

  async function locator_ingest(selector) {
    const res = await page.orig_locator(selector);
    const element_name = "pw_by_selector_" + selector;
    var screenshot = (await page.screenshot()).toString("base64");
    var screenshot_uuid = getScreenshotHash(screenshot);
    try {
      var resp_data = await manager.getIfScreenshotExists(
  ...
  function new_locator_strategy(orig) {
    function closure(selector) {
        return locator_ingest(selector, orig);
    }
    return closure;
  }

  page.locator = locator_ingest;
  page.$ = locator_ingest;
};

module.exports = {
  register_devtools: register_devtools,
};
